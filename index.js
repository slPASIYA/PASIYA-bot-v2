...
function title() {
	  ...
	  console.log(color(figlet.textSync(`${spc2}XCoco`, {
        font: 'Standard',
	  ...
	  console.log(color(`•`, 'aqua'), color(`Owner       : Rey`, 'red'))
	  console.log(color(`•`, 'aqua'), color(`Devoloper    : XCoco`, 'magenta'))
}
// COLOR
colors.setTheme({
warn:'yellow',
      ...
      try {
        ...
        var pp_user = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
            ...
            if (anu.action == 'add') {
                ...
                conn.sendMessage(anu.id, { mentions: [num] , text: `Hii @${num.split("@")[0]} Wellcome To ${metadata.subject}` })
            } else if (anu.action == 'remove') {
                conn.sendMessage(anu.id, { mentions: [num] , text: `@${num.split("@")[0]} Leave!` })
            } else if (anu.action == 'promote') {
                      let metadata = await conn.groupMetadata(anu.id) || ''
                conn.sendMessage(anu.id, { mentions: [num] , text: `@${num.split('@')[0]} Menjadi Admin Di Grup ${metadata.subject}` })
            } else if (anu.action == 'demote') {
                      let metadata = await conn.groupMetadata(anu.id) || ''
                conn.sendMessage(anu.id, { mentions: [num] , text: `@${num.split('@')[0]} Berhenti Menjadi Admin Di Grup ${metadata.subject}` })
            }
		}
		} catch (e) {
  console.log(e)
